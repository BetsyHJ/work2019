...
class Ourmodel(object):
    def __init__(self, n_items, n_users, n_factors, item_factors, user_factors, embedding, learning_rate):
        self.num_neg = 4
    ...
    def _create_variables(self):
        with tf.name_scope('embedding'):
            self.user_map_embedding = tf.get_variable(dtype=tf.float32, initializer=self.user_factors, name='user_map', trainable=False)
    ...
    def read_usermap_value(self):
        return self.user_map_embedding #.eval()
    ...
    def _create_loss(self):
        with tf.name_scope('loss'):
            ...
            self.l2_loss = tf.nn.l2_loss(self.item_emb) + tf.nn.l2_loss(self.user_emb)
...
def training(n_epochs=40, batch_size=256, num_neg=4, resample=True, verbose=10):
    ...
    iidx_images, idx2item, uidx_images, idx2user = data.item_user2image_u2e()
    #idx_images, idx2item = data.item2image()
    #idx_images = idx_images [:, -1:, :]
    #print uidx_images[0]
    #exit(0)
    user2idx = pd.Series(data=np.arange(len(idx2user)), index=idx2user)
    item2idx = pd.Series(data=np.arange(len(idx2item)), index=idx2item)
    n_items, n_factors, embedding = iidx_images.shape
    n_users, _, _ = uidx_images.shape
    
    print "the shape of images for users and items are", uidx_images.shape, iidx_images.shape
    model = Ourmodel(n_items, n_users, n_factors, iidx_images, uidx_images, embedding, 0.01)
    model.build_graph()
    ...
    with tf.Session() as sess:
        ...
        if args.pretrain == True:
            ...
            saver.restore(sess, "./model2/music_GCNEncoder_model.ckpt") # usermap
        ...
        user_map_embedding = model.read_usermap_value().eval() # shape (n, k, d)
        ...
        for iidx in range(item_map_embedding.shape[0]):
            ...
            for k in range(item_flag.shape[0]):
                if iidx in i_us and item_flag[k] <= 0.0:
                    item_map_embedding[iidx, k] = np.mean(user_map_embedding[i_us[iidx], k], axis=0)
...
def get_i_us(rating, item2idx, user2idx):
    i_us = {}
    pos = rating['UserId'].groupby(rating['ItemId'])
    for i, group in pos:
        if i not in item2idx:
            continue
        i_us[item2idx[i]] = [user2idx[x] for x in list(group.values)] # list(group.values)
    return i_us
